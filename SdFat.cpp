...
// macro for debug
#define DBG_FAIL_MACRO DEBUG_MESSAGE("Failed Macro")  //  Serial.println(__LINE__)
...
//==============================================================================
// debug trace macro
#define SD_TRACE(m, b) DEBUG_MESSAGE_TWO(m,b)
...
static uint8_t spiRec(uint8_t * buf, uint16_t nbyte) {
...
bool Sd2Card::init(uint8_t sckRateID, uint8_t chipSelectPin) {
	DEBUG_MESSAGE_TWO("INIT SDCARD AT PIPN ", chipSelectPin);
	DEBUG_MESSAGE_TWO("INIT_SDCARD RateID ", sckRateID);
	errorCode_ = type_ = 0;
...
#endif
	DEBUG_MESSAGE("Init SDCARD Failed!");
	return false;
}
//------------------------------------------------------------------------------
/**
...
bool Sd2Card::setSckRate(uint8_t sckRateID) {
	...
	DEBUG_MESSAGE_TWO("setSckMode:", sckRateID);
	switch (sckRateID) {
...
bool SdVolume::init(Sd2Card * dev, uint8_t part) {
	DEBUG_MESSAGE_TWO("INIT SDCARD ", part);
	...
	if (part) {
		...
		DEBUG_MESSAGE_TWO("Boot Mark=", p->boot);
		DEBUG_MESSAGE_TWO("totalSectors=", p->totalSectors);
		if ((p->boot & 0X7F) != 0 ||
			p->totalSectors < 100 ||
			p->firstSector == 0) {
