...
typedef struct
{
	...
	inline void setEMoveFinished()        //Marcel Endstop
	{
		dir &= ~ESTEP;
	}
...
public:
	...
	inline void checkEndstops()
	{
		if (isCheckEndstops())
		{
			...
			if (isENegativeMove() && Endstops::yMin() /*&& Endstops::xMin()*/) {  //Marcel Endstop neu
				//setEMoveFinished();
			}
			else if (isENegativeMove() && Endstops::yMax() /*&& Endstops::xMax()*/) {
				//setEMoveFinished();
				/*} else if(isENegativeMove() && Endstops::yMin() && Endstops::xMax()){
					setEMoveFinished();
				} else if(isENegativeMove() && Endstops::yMax() && Endstops::xMin()){
					setEMoveFinished(); */
			}
     if (isENegativeMove() && Endstops::e0Max() ) { 
       setEMoveFinished();
      }
