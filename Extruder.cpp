...
static uint8_t tempMessage = 0;
extern uint8_t temp_test_var;
static bool tempTest()//marcel temp
{
	if (Extruder::tempTestFunction(temp_test_var) == 1) {
    //Com::printFLN(PSTR("false test"));
		return false;
	}
	if (Extruder::tempTestFunction(temp_test_var) == 0) {
    //Com::printFLN(PSTR("true test"));
		return true;
	}
}
    ...
    //Com::print(tempTest());
		if (act->currentTemperatureC < MIN_DEFECT_TEMPERATURE || act->currentTemperatureC > MAX_DEFECT_TEMPERATURE)   // no temp sensor or short in sensor, disable heater
		{
			...
			else
			{
				...
				if (tempTest() == true) // marcel temp true für ein, 2 für aus
				{ 
					...
					uid.executeAction(UI_ACTION_TEMP_DEFECT_ERR, false);
				}
			}
		}
...
{
  //Com::printFLN(PSTR("test auf 0"));
	Extruder::tempTestFunction(0); // marcel temptest ein
#if NUM_EXTRUDER > 0
	...
	//UI_CLEAR_STATUS; //marcel preheat test
...
void TemperatureController::updateCurrentTemperature()
{
	...
	case 14:
	{
		...
		DEBUG_MESSAGE("CURRENT TEMP:");
		DEBUG_MESSAGE((int)newtemp);
	}
	break;
	case 13:
