...
extern float newECode;
extern float newCode;
extern float oldCode;
float diff;
float diffVari;
uint8_t countVar;
//float newECode = 0;
...
#endif
uint32_t Printer::interval = 80;           ///< Last step duration in ticks.
...
int32_t Printer::eMaxSteps;                   //Marcel Endstop
...
#endif
#if ((E0_MAX_PIN > -1) && MAX_HARDWARE_ENDSTOP_E0)    //Marcel Endstop
  if (READ(E0_MAX_PIN) != ENDSTOP_E0_MAX_INVERTING)
    newRead2 |= ENDSTOP_E0_MAX_ID;
#endif
...
#endif
#if max_software_endstop_e0            //Marcel Endstop
  //if (Printer::destinationSteps[E_AXIS] > Printer::eMaxSteps) Printer::destinationSteps[E_AXIS] = Printer::eMaxSteps;
#endif
...
void Printer::kill(uint8_t only_steppers)
{
  ...
  else UI_STATUS_UPD_F(Com::translatedF(UI_TEXT_CLICK_START_ID));
#if FAN_BOARD_PIN > -1
#if HAVE_HEATED_BED
...
uint8_t Printer::moveToReal(float x, float y, float z, float e, float f, bool pathOptimize)
{
  ...
  if (e != IGNORE_COORDINATE && !Printer::debugDryrun()
    ...
    destinationSteps[E_AXIS] = e * axisStepsPerMM[E_AXIS];
    //Printer::newECode += e * Printer::invAxisStepsPerMM[E_AXIS];
  if (f != IGNORE_COORDINATE)
    feedrate = f;
...
uint8_t Printer::setDestinationStepsFromGCode(GCode * com)
{
  ...
  if (!relativeCoordinateMode)
  {
    ...
    //if (com->hasE()) lastCmdPos[E_AXIS] = currentPosition[E_AXIS] = convertToMM(com->Z) - coordinateOffset[E_AXIS];
  }
  else
  {
    ...
    //if (com->hasE()) currentPosition[E_AXIS] = (lastCmdPos[E_AXIS] += convertToMM(com->E));
  }
  ...
  if (com->hasE() && !Printer::debugDryrun())
  {
    ...
    //newECode = com->E;
    //Printer::currentPosition[E_AXIS] = convertToMM(com->E);//Printer::currentPositionSteps[E_AXIS] * Printer::invAxisStepsPerMM[E_AXIS];
    //newCode = convertToMM(com->E);
    /*diff = newCode - oldCode;
    newECode += diff;
    oldCode = newCode;*/
    //diffVari = diffVari;
    
    //Printer::currentPosition[E_AXIS] = Printer::currentPosition[E_AXIS] + diff;    
    //Printer::currentPosition[E_AXIS] += newECode;
    //Com::print(Printer::currentPosition[E_AXIS]);
    //if (com->hasE()){
     // if (Printer::relativeCoordinateMode = true){
       // Printer::newECode += com->E;
    ...
    if (/*Printer::currentPosition[E_AXIS]*/Printer::newECode >= /*5*/eMaxPos) {
      uid.executeAction(UI_ACTION_EMPTY_CART_OPEN, false); //Marcel Kartuschenwechsel
#if FEATURE_CONTROLLER == UICONFIG_CONTROLLER     
#endif
      //Com::printFLN(PSTR("NEW cart"));
    }
    /*newECode += com->E - oldCode;
    oldCode = com->E;
    HAL::eprSetFloat(0, newECode);*/
    /*Com::printFLN(PSTR("p: "));
    Com::print(destinationSteps[E_AXIS]/axisStepsPerMM[E_AXIS]);
    Com::printFLN(PSTR("\n newecode: "));
    Com::print(newECode);*/
  }
...
void Printer::setup()
{
  ...
  DEBUG_MESSAGE("Init Print Service");
  HAL::stopWatchdog();
...
#endif
#endif
#if MAX_HARDWARE_ENDSTOP_E0          //marcel Endstop
#if E0_MAX_PIN > -1
  SET_INPUT(E0_MAX_PIN);
#if ENDSTOP_PULLUP_E0_MAX
  PULLUP(E0_MAX_PIN, HIGH);
#endif
#else
#error You have defined hardware e0 max endstop without pin assignment. Set pin number for E0_MAX_PIN
#endif
#endif
...
#error You have defined hardware z max endstop without pin assignment. Set pin number for Z_MAX_PIN
  ...
  Printer::newECode = HAL::eprGetFloat(999);
  HAL::showStartReason();
...
{
  //Com::printFLN(PSTR("im home x haha"));
...
void Printer::homeXAxis()
{
      ...
      (MAX_HARDWARE_ENDSTOP_X && X_MAX_PIN > -1 && X_HOME_DIR == 1 && MAX_HARDWARE_ENDSTOP_Y && Y_MAX_PIN > -1 && Y_HOME_DIR == 1))
  {
    ...
    //Printer::moveToReal(Printer::currentPosition[X_AXIS], Printer::currentPosition[Y_AXIS], -10, IGNORE_COORDINATE, Printer::homingFeedrate[Z_AXIS]);
...
void Printer::homeXAxis()
{
  ...
  //PrintLine::moveRelativeDistanceInSteps(0, 0, axisStepsPerMM[X_AXIS] * 50, 0, homingFeedrate[Z_AXIS], true, false);
  //Printer::moveToReal(Printer::currentPosition[X_AXIS], Printer::currentPosition[Y_AXIS], 5, IGNORE_COORDINATE, Printer::homingFeedrate[Z_AXIS]);
  long steps;
...
{
  //moveToReal(Printer::currentPosition[X_AXIS] - 30, IGNORE_COORDINATE, IGNORE_COORDINATE, IGNORE_COORDINATE, Printer::homingFeedrate[X_AXIS]);
  long steps;
