...
static uint8_t temp_test_var = 1; //marcel temp
class Extruder;
...
//  uint8_t stepPin; ///< Pin number for a step.
	...
	static int tempTestFunction(uint8_t a) { //Marcel temp
		Com::print(a);
    Com::printFLN(PSTR(" in function wert "));
    //HAL::delayMilliseconds(200);
		if (a == 1) {
			return(temp_test_var = 1);
		}
		else {
			if (a == 0) {
				return(temp_test_var = 0);
			}
		}
	}
};
